...
\item
The position of margin notes (and margin figures) can be incorrect when running latex rather than pdflatex. The advice is to use pdflatex whenever possible. If a postscript file is required then run pdflatex before running latex (the pdflatex run will write the correct offsets to the aux file, which latex will use in the subsequent run).
\end{enumerate}
