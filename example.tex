\documentclass[solutionsatend,standalone]{ouunit}
...
\begin{figure*}\attrib{\filename{media/test.png}}
\begin{tikzpicture}
...
\label{fig-test-tikz}\attrib{\filename{media/test.png}}
\begin{tikzpicture}
\draw[axis] (-0.5,0) -- (4,0) node[xlab]{$x$};
\draw[axis] (0,-0.5) -- (0,4) node[ylab]{$y$};
\draw[red,domain={0:3.5}] plot (\x,{(\x)^2/4}) node[black,right] {$y=x^2$};
...
\begin{widefigure}\attrib{\filename{media/test.png}}
...
\lipsum[134]
\begin{marginfigure*}\attrib{\filename{media/icon.png}}
...
\caption{Figure test \label{fig-ex-test}}\attrib{\filename{media/icon.png}}
\begin{tikzpicture}
\draw (0,0) -- (1,1);
\draw (1,0) -- (0,1);
\end{tikzpicture}
...
\item
A numbered margin figure within and unnumbered AMS math environment triggers a spurious `label lost' warning message.
\end{enumerate}
